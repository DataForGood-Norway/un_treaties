...
def links_containing(text):
    def filter(tag):
        return tag.name == "a" and text in tag.get("href", "")
...
def make_df(treaty_soup, url):
    ...
    def resp_table_div(tag):
        return tag.name == "div" and "table-responsive" in tag.get("class", "")
    ...
    for idx, table in enumerate(tables):
        ...
        participant_cols = [
            c for c in df.columns if str(c).lower().startswith("participant")
        ]
        if participant_cols:
            if len(participant_cols) > 1:
                raise ValueError(
                    f"Found more than one participant column: {participant_cols}"
                )
    ...
    # Split PlaceAndDate
    df["TreatyPlace"], _, df["TreatyDate"] = zip(
        *[
            [x.strip() for x in d.partition(",")]
...
def _convert_field(data_s):
    re_field = re.compile(
        "(ratification|acceptance|accession|succession|definitive signature|application|adoption|registration date|notification)[^,()]*(?:\((\w+)\))?",
        flags=re.IGNORECASE,
    )
...
def _convert_date(data_s):
    re_date = re.compile(
        "(\d\d? (jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\w* \d{4}) *(\w*)",
        flags=re.IGNORECASE,
    )
